...
- **Ensemble replicates** (`dock_ensemble`): each repeat uses `base_seed + i`. When `seed=None` (default) the base is drawn from the OS CSPRNG and recorded in `summary["base_seed"]`, so two unseeded ensembles are genuinely independent yet still reproducible after the fact; pass an explicit `seed` only for bit-for-bit reproducibility.
- **Batch naming** (`autodock batch`): receptor/ligand display names, output directories, and the heatmap are all keyed by **file basename**. Every input file must have a unique basename (duplicate basenames abort with an error). For post-processing (interactions + figures), each receptor PDBQT must have a same-basename `.pdb`/`.cif` in `--receptor-pdb-dir`; otherwise that receptor's pairs dock without interaction analysis (a WARNING is logged).
...
- **Figures**: 3D scenes (complex/pocket/interaction) need PyMOL; 2D interaction diagram needs RDKit. All 3D PNGs have solid (non-transparent) **white** backgrounds with black labels (default `publication_white` colour scheme; `presentation_black` is still available). `3d_complex.png` is the standalone whole-receptor view for user compositing. The interaction scene shows an 8 Å pocket window with side-chain sticks; dashed interaction lines are drawn per ligand atom and colored by type (cyan H-bond, orange hydrophobic, green π-π, purple π-cation, red salt bridge, …) with the distance labelled on the closest pair; interacting residues get prominent bold labels and surrounding pocket residues smaller dim ones. The 2D diagram clips connectors at label borders, reserves the bottom-right legend from label placement, and scales all legend/label geometry to the canvas; multi-ring ligands get light-grey fill on non-interacting aromatic rings. With `interaction_method="both"` (default), ProLIF's native figures are also written to `03_figures/`: `2d_prolif_network.html` (interactive LigNetwork — the canonical ProLIF 2D interaction figure) and `2d_prolif_barcode.png/pdf` (residue × interaction-type barcode). All figures are 300 dpi (3D ray-traced 2400×1800 px, whole-complex 3200 px wide; 2D PNG 5400×4200 px) with PDF counterparts. Verify figure quality visually before publication — do not ship unviewed PNGs.
